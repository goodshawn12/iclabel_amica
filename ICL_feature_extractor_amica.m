...
function features = ICL_feature_extractor_amica(EEG, model, flag_autocorr)
%% check inputs
...
%% calc psd
psd = eeg_rpsd_amica(EEG, model, 100);
...
if flag_autocorr
% [TODO] Implement "eeg_autocorr" and "eeg_autocorr_fftw"?
%     if EEG.trials == 1
%         if EEG.pnts / EEG.srate > 5
%             autocorr = eeg_autocorr_welch_amica(EEG, model);
%         else
%             disp 'ICL_feature_extractor: '
...
%     end
    ...
    % computed autocorr using weighted psd
    autocorr = eeg_autocorr_welch_amica(EEG, model);
