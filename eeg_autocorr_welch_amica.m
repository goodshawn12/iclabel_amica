function ac = eeg_autocorr_welch_amica(EEG, model, pct_data)
...
% setup constants
len_seg = 3;    % times of (0.5 sec)-segment 
ncomp = size(EEG.icaweights, 1);
n_points = min(EEG.pnts, EEG.srate * len_seg);
...
% compute model probability smoothed over "len_seg" segments
nBlock = size(EEG.etc.amica.smModProb,2);
temp = reshape(EEG.etc.amica.smModProb(model,1:floor(nBlock/len_seg)*len_seg),len_seg,[]);
temp = mean(temp,1);
temp = temp(1,1:size(index,2));

%% calc autocorrelation
fftpow = abs(fft(segments, nfft, 2)).^2;

% calculate average PSD weighted by windowed model probability
fftpow = bsxfun(@times, fftpow, reshape(temp,1,1,[]));
ac = ifft(sum(fftpow,3)/sum(temp), [], 2);
